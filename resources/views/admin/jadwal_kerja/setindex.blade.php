...
//                dTableKar.ajax.reload();
                e.preventDefault();
                let frm = document.getElementById('frmTransSetJadwal');
                let datas = new FormData(frm);
//                console.log($('#form_data_upload').attr('action'));
                $.ajax(
                {
                    url         : $('#frmTransSetJadwal ').attr('action'),
                    dataType    : 'JSON',
                    type        : 'POST',
                    data        : datas ,
                    processData: false,
...
//                        $('#loadingDialog').modal('show');
            ...
            dTableKar = $('#dTableKar').DataTable({
                ...
                },
                {
                        ...
                        data: function(data)
                        {
                            ...
                            {
                                return data.jadwals[0].kode+" - "+data.jadwals[0].tipe;
                            }
                            else
                            {
                                return '';
                ...
                },
                {
                        ...
                        data: function(data)
                        {
                            if(data.jadwals.length > 0)
                            {
                                return data.jadwals[0].pivot.tanggal;
                            }
                            else
                            {
                                return '';
        ...
        var detFormat = function(dt)
        {
            if(dt.jadwals.length > 0)
            {
                ...
                dt.jadwals.forEach(function(i,x)
                {
...
<div class="modal fade" id="modal-form-upload">
    <div class="modal-dialog">
        <div class="modal-content bg-secondary">
            ...
            <h4 class="modal-title"><i class="fa fa-upload"></i>Form Upload Jadwal Karyawan</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
<!--        <form id="form_data" action="{{route('savejadwalday')}}" accept-charset="UTF-8" >-->
            {{ Form::open(['route' => ['uploadjadwalkaryawan'], 'id' => 'form_data_upload', 'files' => true]) }}
            ...
            <div class="modal-body">   
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-12">
                            <a class="btn btn-info btn-xs" href="{{route('app.files', 'file_temp_karyawan_jadwal')}}" target="_blank"><i class="fa fa-download"></i>Template Document</a>
                        </div>
                    </div>
                </div>
            </div>   
                    ...
                    <div class="card-header">
                        {{Form::open(['url' => route('savejadwalset'),'class'=>'form-data', 'id' => 'frmTransSetJadwal'])}}
                            <div class="row">
                                <div class="col-2">
                                    <div class="form-group">
                                        {{ Form::label('sTanggal', 'Tanggal Rubah') }}
                                        <div class="input-group" data-target-input="nearest">
                                            {{ Form::date('sTanggal', now(), ['id' => 'sTanggal', 'class' => 'form-control form-control-sm', 'placeholder' => 'Tanggal Alasan']) }}
