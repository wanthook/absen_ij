...
<body>
@if(isset($var))
    <div class="j1">Laporan Karyawan Mangkir/TA</div>
    <div class="j2">Periode : {{$periode}}</div>
    <table class="detail">
        <thead>
            <tr>
                <th>No</th>
                ...
                <th>Tanggal<br>Tidak Absen</th>
                <th>Jadwal<br>Kerja</th>
                <th>Jam<br>Kerja</th>
                <th>Keterangan</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="dc">{{isset($vVar['tanggal_masuk'])?$vVar['tanggal_masuk']:''}}</td>
                <td class="dc">{{isset($vVar['kode_divisi'])?$vVar['kode_divisi']:''}}</td>
                <td class="dc">{{isset($vVar['nama_divisi'])?$vVar['nama_divisi']:''}}</td>
                <td class="dc">{{isset($vVar['tanggal'])?$vVar['tanggal']:''}}</td>
                <td class="dc">{{isset($vVar['jadwal_kerja'])?$vVar['jadwal_kerja']:''}}</td>
                <td class="dc">{{isset($vVar['jam_kerja'])?$vVar['jam_kerja']:''}}</td>
